...
if not TMDB_API_KEY:
    print("WARNING: TMDB_API_KEY missing. TMDB endpoints disabled (local TF-IDF works). Add to .env.")
...
async def tmdb_get(path: str, params: Dict[str, Any], retries: int = 3) -> Dict[str, Any]:
    """
    Safe TMDB GET with retries:
    - Timeout 60s, 3 retries with backoff
    """
    if not TMDB_API_KEY:
        raise HTTPException(status_code=503, detail="TMDB disabled - set TMDB_API_KEY in .env")
    q = dict(params)
    ...
    for attempt in range(retries):
        try:
            ...
            if r.status_code == 200:
                return r.json()
            raise HTTPException(status_code=502, detail=f"TMDB {r.status_code}: {r.text[:200]}")
        except httpx.TimeoutException as e:
            if attempt == retries - 1:
                raise HTTPException(status_code=502, detail=f"TMDB timeout after {retries} tries: {e}")
        except httpx.RequestError as e:
            if attempt == retries - 1:
                raise HTTPException(status_code=502, detail=f"TMDB error: {type(e).__name__}: {e}")
        if attempt < retries - 1:
...
async def home(
    ...
    """
    Home feed: TMDB (retry/fallback) or local TF-IDF top movies.
    """
    try:
        ...
        # Fallback: Local top TF-IDF movies (no TMDB needed)
        global df
        if df is None:
            return []
        # Sample diverse top movies (simulate popular)
        sample_idx = df.sample(min(limit, len(df)), random_state=42).index.tolist()
        fallback = []
        for i in sample_idx:
            title = df.iloc[i]['title']
            fallback.append(TMDBMovieCard(
                tmdb_id=-1,  # placeholder
                title=title,
                poster_url=None,
                release_date=None,
                vote_average=None
